...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  error_derivative=(class_prob-data.targets)./ size(data.inputs, 2);
  %size: <number of classes, i.e. 10> by <number of data cases>
  ret.hid_to_class=error_derivative*hid_output'+wd_coefficient*model.hid_to_class;
  % size:<number of classes, i.e. 10> by  <number of hidden units>  modify ret.hid_to_class  
  %number of cases are added up,because it's in the middle of matrix multiplication
  
 %hid_output size: <number of hidden units> by <number of data cases>
  %model.hid_to_class size:<number of classes i.e. 10> by <number of hidden units>
  %error_derivative size:<number of classes, i.e. 10> by <number of data cases>
  backperror1=(model.hid_to_class'*error_derivative).*hid_output.*(1-hid_output);
 %backperror1 size <number of hidden units> <number of data cases>
 
   %hid_input size: <number of hidden units> by <number of data cases>
   % data.inputs is a matrix of size <number of inputs i.e. 256> by <number of data cases>
  ret.input_to_hid=backperror1*data.inputs'+wd_coefficient*model.input_to_hid;
   %ret.input_to_hid size <number of hidden units> by <number of inputs i.e. 256>
...
function q8_wd(w)
    wds = [1,10,0.001,0,0.0001,0.1];
    for i = 1:size(wds,2),
      a3(wds(i), 200, 1000, 0.35, 0.9, false, 100);
    endfor
end

function q9_hn(w)
    hns = [130, 10, 170, 30, 100];
    for i = 1:size(hns,2),
      a3(0, hns(i), 1000, 0.35, 0.9, false, 100);
    endfor
 end
